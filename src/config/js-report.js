...
// js-tracking
// 错误收集，线上和beta环境
if (__PROD__ && !env.console && env.isEnv(['prod', 'beta'])) {
  // 个人版 3000错误事件/月 数据保存7天
  const jsTracking = require('fundebug-javascript')
  jsTracking.apikey = '2a6da749751f004cf0d63f9c93049a498131b519b29fbc8beeaefa6580afb3ca'
  ...
  debugReport = (debugString) => {
    if (!caches[debugString]) {
      ...
      console.log('js-report:', debugString)
      jsTracking.notify('js-report:', debugString)
    }
  }
  ...
  function formatComponentName(vm) {
    ...
    const name = vm._isVue ? (vm.$options && vm.$options.name) ||
