...
if (android) os.android = true, os.version = android[2], device.androidChrome = !!(ua.toLowerCase().indexOf('chrome') >= 0)
...
// custom host
let alipay = ua.match(/(AliApp\(AP)\/([\d.]+)/) || ua.match(/(AlipayClient)\/([\d.]+)/)
...
if (os.ios && os.version && ua.indexOf('Version/') >= 0) {
  if (os.version.split('.')[0] === '10') {
    os.version = ua.toLowerCase().split('version/')[1].split(' ')[0]
  }
}
...
if (os.name === 'ios') {
  ...
  device.minimalUi = !webview &&
      (iphone || ipod) &&
      (versionArr[0] * 1 === 7 ? versionArr[1] * 1 >= 1 : versionArr[0] * 1 > 7) &&
